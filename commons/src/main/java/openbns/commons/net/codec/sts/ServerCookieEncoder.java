...
import static openbns.commons.net.codec.sts.CookieEncoderUtil.*;
/**
    ...
    public static String encode(Cookie cookie) {
        ...
        if (cookie.getMaxAge() != Long.MIN_VALUE) {
            if (cookie.getVersion() == 0) {
                CookieEncoderUtil.addUnquoted( buf, CookieHeaderNames.EXPIRES, HttpHeaderDateFormat.get().format( new Date( System.currentTimeMillis() + cookie.getMaxAge() * 1000L ) ) );
            } else {
        ...
        if (cookie.getPath() != null) {
            if (cookie.getVersion() > 0) {
                ...
                CookieEncoderUtil.addUnquoted( buf, CookieHeaderNames.PATH, cookie.getPath() );
            }
        }
        ...
        if (cookie.getDomain() != null) {
            if (cookie.getVersion() > 0) {
                ...
                CookieEncoderUtil.addUnquoted( buf, CookieHeaderNames.DOMAIN, cookie.getDomain() );
            }
        }
        if (cookie.isSecure()) {
        ...
        if (cookie.getVersion() >= 1) {
            ...
            CookieEncoderUtil.add( buf, CookieHeaderNames.VERSION, 1 );

            if (cookie.getCommentUrl() != null) {
                CookieEncoderUtil.addQuoted( buf, CookieHeaderNames.COMMENTURL, cookie.getCommentUrl() );
            }
